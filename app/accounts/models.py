...
class Goal(db.Model):
    ...
    # VALIDATION - ENUM - Hobby/Travel/Marriage/ChildEd/Retirement
...
class MarketUnit(db.Model):
    ...
    Description    :   Curent and history value for all market linked units
                                    One special unit to denote INR- to expand when using other currencies
    ...
    id=db.Column(db.Integer, primary_key=True)
    urn=db.Column(db.String(16))  # actual symbol- IOC(NSE symbol), INR(currency), 113578(AMFI ID)

    # "NSE symbol"/BSE Symbol/NASDAQ Symbol/AMFI ID/Currency
    ...
    #PROPERTY - unit_value in base currency terms

    # constraint - at any point only one urn should be present which is active
...
class Transaction(db.Model):
    ...
    trans_status =db.Column(db.String(16))   # scheduled/inprogress/completed/failed/reversed
    #PROPERTY - trans_value in base currency terms
